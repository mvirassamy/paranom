...
export const UiButton = ({title, onClick, size = SizeButton.S, color = "blue", variation = "900", loading = false}) => {
    return (
        <button
            className={`middle none z-30 center rounded-lg bg-${color}-${variation} ${size} font-sans text-xs font-bold uppercase text-white shadow-md shadow-${color}-500/20 transition-all hover:shadow-lg hover:shadow-${color}-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none`}
